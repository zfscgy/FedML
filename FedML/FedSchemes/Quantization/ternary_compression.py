...
from FedML.Models.SpecialModels.trainable_ternarize_models import TernarizedModel
...
class TrainableTernary:
    ...
    def global_ternarize(tensors: Union[List[torch.Tensor]], excluded_indices: List[int] = None):
        ...
        for i in range(len(tensors)):
            if i not in excluded_indices:
                ternaried_tensors.append(TrainableTernary.ternarize_by_batch(tensors[i], n_batch_dims=1))
...
class TernaryServer_Trainable(FedAvgServer):
    ...
    def update(self):
        clients = np.random.choice(self.clients, self.options.n_clients_per_round)
        self.current_training_clients = clients
        ternarized_global_valus = self.options.ternarize_server(self.global_model.get_ternarized_values())[0]
        received_weights = []
        for client in clients:
            self.sended_size += self.n_tensor_elements * self.compress_rate
            client.local_model.set_ternarized_values(ternarized_global_valus)
            client.update()
