...
from FedML.FedSchemes.Compression.base_compressor import BaseCompressionClientOptions, BaseCompressionServerOptions
...
from FedML.FedSchemes.Compression.sparse_ternary_compression import STC, STCClient, STCServer
from FedML.FedSchemes.Compression.sign_sgd import SignSGDClient, SignSGDServerOptions, SignSGDServer, \
...
get_model = lambda: Convert.model_to_device(LeNet5())
...
def fed_avg_test():
    ...
    for i in range(n_clients):
        client = FedAvgClient(
            ...
            server,
            FedAvgClientOptions(
                client_data_loader=DataLoader(iid_mnist_datasets[i], batch_size=local_batch_size),
    ...
    for i in range(n_clients):
        client = STCClient(
            ...
            BaseCompressionClientOptions(
                client_data_loader=DataLoader(iid_mnist_datasets[i], local_batch_size),
                get_optimizer=get_optimizer,
                loss_func=loss_func,
                batch_mode=True,
                n_local_rounds=n_local_rounds,
...
def signsgd_test():
    ...
    server = SignSGDServer(
        get_model,
        SignSGDServerOptions(
            n_clients_per_round=10,
            learning_rate=1e-3,
            momentum=0.9
        )
    ...
    client_model = get_model()
    for i in range(n_clients):
        client = SignSGDClient(
            lambda: client_model,
            server,
            FedAvgClientOptions(
                client_data_loader=DataLoader(iid_mnist_datasets[i], batch_size=local_batch_size),
                get_optimizer=lambda m: SGD(m.parameters(), 1),
                loss_func=loss_func,
                batch_mode=True,
                n_local_rounds=1,
            )
